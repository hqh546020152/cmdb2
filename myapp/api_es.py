        ...
        try:
            if request.session['username']:
                try:
                    ...
                    stats = request.POST.get('stats', '运行中')
                    ...
                    affiliation = request.POST.get('affiliation','阿里云')



                    message = {'cpu': cpu, 'tagname': tagname, 'memory': memory, 'i_ip': i_ip, 'e_ip': e_ip, 'ip': ip,
                               'systemd_version': systemd_version,
                               'kernel_version': kernel_version, 'tag': tag, 'create_time': create_time, 'space': space,
                               'stats': stats, 'location':location, 'sshport': sshport, 'affiliation': affiliation,
        ...
        try:
            if request.session['username']:
                ...
                if id:
                    ...
                    message = {'cpu': cpu, 'tagname': tagname, 'memory': memory, 'ip': ip, 'e_ip': e_ip, 'i_ip': i_ip,
                               'systemd_version': systemd_version, 'location': location, 'sshport':sshport, 'affiliation': affiliation,
